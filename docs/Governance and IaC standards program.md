...
| G2 | A client is defined, and the naming convention is formal and documented | Client `lex` — Lexfield Legal, a fictitious law firm and the client for which GaCode Solutions, the team's own company, builds MicroTodoSuite (first recorded as `gcs` and corrected by the maintainer the same day) — project `mts`, environments `shd`, `eco`, `fdev`, `fstg`, `fprd` — `rules/iac/MTS-IAC-101` in the AI repository |
